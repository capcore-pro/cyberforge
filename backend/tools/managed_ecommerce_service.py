"""
Managed ecommerce provisioning.

V1: deploy frontend-only on Vercel; ecommerce API is served by CyberForge backend.
"""
...
class ManagedEcommerceError(Exception):
    pass
...
def _template_files(slug: str) -> dict[str, str]:
    try:
        ...
        template_dir = root / "templates" / "ecommerce-next"
        ...
        if "package.json" in files:
            files["package.json"] = files["package.json"].replace(
                "\"name\": \"ecommerce-next\"",
                f"\"name\": \"{slug}-ecommerce\"",
            )
        return files
    except Exception as exc:
        logger.warning("ecommerce template fallback used: %s", exc)
...
async def provision_ecommerce(
    *,
    ...
    if not project:
        raise ManagedEcommerceError("Projet introuvable.")
    ...
    try:
        ...
        vercel_project_id = vercel_project_id or await ensure_project_for_github_branch(
            ...
            root_directory=None,
            env={
                "NEXT_PUBLIC_ECOMMERCE_API_BASE_URL": resolved.demo_api_base_url,
                "NEXT_PUBLIC_ECOMMERCE_SLUG": slug,
            },
            settings=resolved,
        )
...
async def hard_delete_ecommerce(
    *,
    project_id: str,
    settings: Settings | None,
    store: ManagedProjectsStore,
