...
Heuristiques d??terministes uniquement (BuilderAI v2).
"""
...
# Jeton TYPE: ??? (ProjectType, cat??gorie tarifaire optionnelle, generation_mode optionnel)
...
def parse_type_prefix(prompt: str) -> tuple[str | None, str]:
    """
    Extrait TYPE: <jeton> en t??te de prompt.
    Retourne (jeton normalis??, corps du prompt sans la directive TYPE).
    """
    text = prompt.strip()
...
def resolve_forced_type_token(token: str) -> tuple[ProjectType, PricingCategory | None, str | None]:
    """Interpr??te le jeton TYPE: (ProjectType, cat??gorie tarifaire, mode g??n??ration)."""
    ...
    try:
        ...
        raise ValueError(
            f"Type inconnu apr??s TYPE: ?? {token} ??. Valeurs accept??es : "
            + ", ".join(valid)
        ) from exc
...
class ArchitectPlan(BaseModel):
    ...
    secteur: str | None = Field(
        default=None,
        description="Secteur toolbox d??tect?? (restauration, immobilier, ???)",
    )
    ...
    complexity_score: int = Field(
        ge=1,
        le=10,
        description="Score de complexit?? prompt (1???10)",
    )
    complexity_label: str = Field(
        description='Libell?? : "Simple", "Moyenne" ou "Complexe"',
    )
    market_price_min: int = Field(ge=0, description="Fourchette march?? basse (???)")
    market_price_max: int = Field(ge=0, description="Fourchette march?? haute (???)")
    suggested_price_min: int = Field(ge=0, description="Prix de vente sugg??r?? bas (~40 % march??)")
    suggested_price_max: int = Field(ge=0, description="Prix de vente sugg??r?? haut (~40 % march??)")
    pricing_category: str = Field(
        default="application_web",
        description="Cat??gorie tarifaire (vitrine_next, ecommerce, ???)",
    )


class ArchitectAgent(BaseAgent):
    """S??lectionne type de projet + template avant la g??n??ration (heuristiques)."""
    ...
    async def plan_with_analysis(
        ...
        *,
        ...
        if (
            ...
            # Carte ?? E-commerce ?? du g??n??rateur (project_type saas_dashboard).
        ...
        type_rationale = (
            f"Type impos?? par TYPE: ?? {forced_token} ?? ??? ?? {type_label} ??. "
            if forced_token
            else f"Type ?? {type_label} ??. "
        )
        rationale = (
            f"{type_rationale}"
            f"Complexit?? {pricing['complexity_label']} {pricing['complexity_score']}/10. "
            f"Template premium ?? {template_label} ?? choisi par analyse heuristique du prompt. "
            f"March?? estim?? {pricing['market_price_min']}???{pricing['market_price_max']} ???."
        )
        plan = ArchitectPlan(
    ...
    def _apply_toolbox(
        ...
        """Charge GET /toolbox/secteur/{secteur} (donn??es locales) et enrichit le plan."""
        ...
        toolbox_note = (
            f" Secteur toolbox ?? {bundle.nom} ?? ??? palette et composants transmis ?? BuilderAI."
        )
