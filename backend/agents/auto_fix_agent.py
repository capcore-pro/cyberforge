...
AutoFixAI ΓÇö corrige les livrables HTML d├⌐fectueux.

Repli align├⌐ sur le template ArchitectAI (TaskFlow uniquement pour SaaS explicite).
...
def needs_immediate_template_fallback(report: BugHuntReport) -> bool:
    ...
    """True si le livrable ne doit pas passer par des r├⌐g├⌐n├⌐rations LLM."""
...
class AutoFixAgent(BaseAgent):
    ...
    R├⌐g├⌐n├¿re via LLM puis repli template premium (ArchitectAI).

    Vitrines : reprise BuilderAI simplifi├⌐e.
    ...
    async def repair(
        ...
        *,
        ...
        R├⌐g├⌐n├⌐ration LLM puis repli template ArchitectAI si ├⌐chec.
        ...
        if needs_immediate_template_fallback(initial_report):
            ...
            logger.info(
                ...
                "[AutoFixAI] repli imm├⌐diat | issues=%s | vitrine=%s",
            ...
            return await self._resolve_fallback(
                ...
                reason="code source ou rendu cass├⌐ d├⌐tect├⌐",
        ...
        for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
            ...
            try:
                ...
                logger.exception("[AutoFixAI] ├⌐chec appel LLM tentative %s", attempt)
            ...
            if needs_immediate_template_fallback(last_report):
                ...
                return await self._resolve_fallback(
                    ...
                    reason="r├⌐g├⌐n├⌐ration encore d├⌐fectueuse",
            ...
            if last_report.ok:
                ...
                logger.info("[AutoFixAI] HTML valid├⌐ apr├¿s tentative %s", attempt)
                ...
                return (
                    ...
                    code_result_from_html(
                        ...
                        summary=(
                            ...
                            f"{generation.summary} (corrig├⌐ par AutoFixAI, "
        ...
        logger.warning(
            ...
            "[AutoFixAI] ├⌐chec apr├¿s %s tentatives ΓÇö repli template ArchitectAI",
        ...
        return await self._resolve_fallback(
            ...
            reason=f"├⌐chec apr├¿s {MAX_FIX_ATTEMPTS} tentatives",
