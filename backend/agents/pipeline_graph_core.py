"""
Pipeline LangGraph complet ��� ArchitectAI ��� �Ǫ ��� TestPilotAI ��� ExportAI ��� Finalisation.
ExportAI d%#ploie sur Cloudflare Pages, Railway ou GitHub selon le type de projet.
"""
...
# %�tapes optionnelles : research (settings.research_enabled).
# Identifiants de n<%�uds LangGraph (��� cl%#s du state TypedDict)
...
# Labels SSE sp%#cifiques au mode "Vraie app"
REAL_APP_STEP_MESSAGES: dict[str, str] = {
    "coremind_start": "G%#n%#ration app React/TypeScript�Ǫ",
    "export_start": "D%#ploiement application React (Railway / Vercel)�Ǫ",
}
...
class PipelineState(TypedDict, total=False):
    ...
    # "client_demo" (d%#faut) ��� pipeline HTML premium ; "real_app" ��� React/Next.js
...
def is_vitrine_generation_mode(state: PipelineState) -> bool:
    """True pour les modes vitrine HTML assembl%#e (pas real_app)."""
...
def _architect_input_prompt(state: PipelineState) -> str:
    ...
    if not base:
        return brief
    return (
        "## Brief d'inspiration (site source analys%#)\n\n"
        f"{brief}\n\n"
        "## Prompt utilisateur\n\n"
        f"{base}"
    )
...
async def architect_node(
    ...
    await _step(cb, "architect", "start", "Analyse du prompt et choix du template�Ǫ")
    ...
    await _step(
        ...
        "done",
        (
            f"{plan.project_type_label} ,%V% template {plan.template_label} ,%V% "
            f"{plan.complexity_label} ({plan.complexity_score}/10) ,%V% "
            f"march%# {plan.market_price_min}���{plan.market_price_max} ��"
        ),
...
def _should_run_database_ai(state: PipelineState) -> bool:
    """
    Ex%#cute DatabaseAI apr%�s ArchitectAI sauf:
...
async def database_ai_node(
    ...
    cb = _callback_from_config(config)
    await _step(cb, NODE_DATABASE, "start", "G%#n%#ration du sch%#ma Supabase�Ǫ")
    plan = state.get("architect_plan")
    if not plan:
        await _step(cb, NODE_DATABASE, "done", "Plan ArchitectAI manquant ��� %#tape ignor%#e.", ok=True)
        return {}
    ...
    await _step(
        ...
        NODE_DATABASE,
        "done",
        str(schema.get("summary") or "Sch%#ma Supabase g%#n%#r%#."),
        ok=True,
    )
...
async def auth_ai_node(
    ...
    cb = _callback_from_config(config)
    await _step(cb, NODE_AUTH, "start", "G%#n%#ration du sch%#ma Auth/RLS�Ǫ")
    ...
    # Skip si public (contrat demand%#)
    if (schema.get("auth_type") or "").strip().lower() == "public":
        await _step(cb, NODE_AUTH, "done", "Auth public ��� %#tape ignor%#e.", ok=True, skipped=True)
        return {}
    ...
    await _step(
        ...
        NODE_AUTH,
        "done",
        str(schema.get("summary") or "Sch%#ma Auth/RLS g%#n%#r%#."),
        ok=True,
    )
...
async def payment_ai_node(
    ...
    cb = _callback_from_config(config)
    await _step(cb, NODE_PAYMENT, "start", "G%#n%#ration config paiements (Stripe)�Ǫ")
    try:
    ...
    if (result.get("payment_type") or "").strip().lower() == "none":
        await _step(cb, NODE_PAYMENT, "done", "Paiement non requis ��� %#tape ignor%#e.", ok=True, skipped=True)
        return {}

    payment_type = str(result.get("payment_type") or "").strip().lower()
    logger.info("[PaymentAI] Stripe configur%#: type=%s", payment_type)
    await _step(
        cb,
        NODE_PAYMENT,
        "done",
        str(result.get("summary") or "Configuration paiements g%#n%#r%#e."),
        ok=True,
    )
...
async def electron_ai_node(
    ...
    cb = _callback_from_config(config)
    await _step(cb, NODE_ELECTRON, "start", "G%#n%#ration du projet Electron (Windows)�Ǫ")
    try:
    ...
    await _step(
        ...
        NODE_ELECTRON,
        "done",
        str(result.get("summary") or "Fichiers Electron g%#n%#r%#s."),
        ok=True,
    )
...
async def design_system_node(
    ...
    """DesignSystemAI ��� loi visuelle apr%�s Research, avant Template / Content."""
    cb = _callback_from_config(config)
    await _step(cb, NODE_DESIGN_SYSTEM, "start", "G%#n%#ration du design system�Ǫ")
    plan = state.get("architect_plan")
    if not plan:
        return {
    ...
    await _step(
        ...
        NODE_DESIGN_SYSTEM,
        "done",
        f"Palette {doc.colors.primary} ,%V% {doc.fonts.heading} / {doc.fonts.body}",
...
async def template_generator_ai_node(
    ...
    """TemplateGeneratorAI ��� HTML sectoriel g%#n%#r%# par Claude ; repli catalogue si %#chec."""
    cb = _callback_from_config(config)
    ...
    await _step(
        ...
        NODE_TEMPLATE_GENERATOR,
        "start",
        "G%#n%#ration du template HTML (Claude Sonnet)�Ǫ",
    )
    ...
    logger.warning(
        "[TemplateGeneratorAI] %#chec g%#n%#ration ��� repli catalogue sectoriel"
    )
    await _step(
        cb,
        NODE_TEMPLATE_GENERATOR,
        "start",
        "Repli sur le template sectoriel du catalogue�Ǫ",
    )
    ...
    return fallback if fallback is not None else {"error": "TemplateGeneratorAI : %#chec."}
...
async def template_ai_node(
    ...
    """TemplateAI ��� charge le HTML sectoriel de base (loi visuelle d%#j%� fix%#e)."""
    cb = _callback_from_config(config)
    ctx = _template_context_from_state(state)
    if ctx is None:
    ...
    await _step(cb, "template_ai", "start", "Chargement du template sectoriel�Ǫ")

    logger.info(
        "[TemplateAI] entr%#e | project_type=%s | pricing_category=%s | sector=%s | prompt_len=%d",
        pt_value,
        pricing_cat or "?",
        sector,
        len(base_prompt),
    ...
    return fallback if fallback is not None else {"error": "TemplateAI : %#chec."}
...
async def content_ai_node(
    ...
    """ContentAI ��� contenu client r%#el dans le template sectoriel."""
    cb = _callback_from_config(config)
    plan = state.get("architect_plan")
    ...
    await _step(cb, "content_ai", "start", "R%#daction du contenu client�Ǫ")
    ...
    filled_preview, filled_assembled = resolve_pipeline_preview_html(
        ...
        title=plan.project_type_label if plan else "D%#mo CyberForge",
        user_prompt=base_prompt,
    )
    await _step(
        cb,
...
def _route_after_architect(state: PipelineState) -> str:
    """ResearchAI (si activ%#) juste apr%�s Architect ��� avant loi visuelle."""
    if state.get("error"):
        return "finalize"
...
async def extension_build_node(
    ...
    await _step(cb, "extension_build", "start", "G%#n%#ration extension Chrome MV3�Ǫ")
    ...
    try:
        ...
        logger.exception("[extension_build] %#chec")
...
async def research_node(
    ...
    plan = state.get("architect_plan")
    if not plan:
        return {"error": "%�tat pipeline incomplet (architect_plan manquant pour ResearchAI)."}
    ...
    await _step(
        ...
        f"({ctx['nom_entreprise'] or 'projet'})�Ǫ",
    )
    settings = _settings_from_config(config)
    ...
    if brief.skipped or not brief.enriched:
        await _step(
            ...
            "research",
            "done",
            brief.skip_reason or "Recherche ignor%#e ��� g%#n%#ration sans brief externe.",
            ok=True,
    ...
    if isinstance(fc, dict) and (fc.get("palette") or fc.get("couleurs")):
        logger.info(
            "[Firecrawl] firecrawl_result pr%#sent apr%�s ResearchAI ��� pr%�t pour DesignSystemAI"
        )
    ...
    await _step(
        ...
        f"{len(brief.mots_cles)} mot(s)-cl%#(s).",
        ok=True,
...
async def openhands_node(
    ...
    if not plan or not analysis:
        return {"error": "%�tat pipeline incomplet (architect_plan manquant pour OpenHands)."}

    await _step(
        cb,
        "openhands",
        "start",
        f"G%#n%#ration avanc%#e OpenHands (complexit%# {plan.complexity_score}/10)�Ǫ",
    )
    settings = _settings_from_config(config)
    ...
    await _step(
        ...
        "openhands",
        "done",
        f"G%#n%#ration OpenHands r%#ussie ({result.provider}).",
        provider=result.provider,
        fallback=False,
    )
    return {
...
async def builder_node(
    ...
    analysis = state.get("analysis")
    if not plan or not analysis:
        return {"error": "%�tat pipeline incomplet (architect_plan manquant pour BuilderAI)."}

    await _step(cb, "builder", "start", "Assemblage template ou g%#n%#ration v0/DeepSeek�Ǫ")
    settings = _settings_from_config(config)
    ...
    await _step(
        ...
        "builder",
        "done",
        f"G%#n%#ration via {provider_label} r%#ussie.",
        provider=result.decision.provider.value,
        fallback=False,
...
def _route_after_builder(state: PipelineState) -> str:
    ...
    if state.get("builder_fallback", True):
        if _block_coremind_builder_fallback(state):
            logger.error(
                "[BuilderAI] fallback CoreMind BLOQU%� ��� template sectoriel requis "
                "(category=%s)",
...
def _inject_package_json(
    ...
    *,
    ...
    """
    Ajoute package.json + src/main.tsx + index.html si absents de la g%#n%#ration
    pour produire un projet React/Vite d%#ployable sur Railway ou Vercel.
    """
...
async def coremind_node(
    ...
    analysis = state.get("analysis")
    if not plan or not analysis:
        return {"error": "%�tat pipeline incomplet (architect_plan manquant)."}
    ...
    if not state.get("builder_fallback", True) and state.get("generation"):
        await _step(
            ...
            "coremind",
            "done",
            "G%#n%#ration d%#j%� fournie par BuilderAI ��� %#tape ignor%#e.",
            skipped=True,
        )
        return {}
    ...
    if generation_mode == "vitrine_next":
        ...
        try:
            ...
            return {"error": f"G%#n%#ration vitrine : {exc}"}
        await _step(
            cb,
            "coremind",
            "done",
            (
                f"Scaffold vitrine ��� {vitrine.content.meta.businessName} ,%V% "
                f"{vitrine.file_count} fichier(s)"
                + (
                    f" ,%V% {vitrine.images_resolved} image(s) Unsplash"
    ...
    # --- Mode "Vraie app" : g%#n%�re un projet React/TypeScript complet ---
    if generation_mode == "real_app":
        await _step(
            cb,
            "coremind",
            "start",
            f"G%#n%#ration application React/TypeScript ({type_label})�Ǫ",
        )
        codegen = CodeGenService(settings)
        tier = CodeGenComplexity(analysis.complexity.value)
        enriched_real = (
            f"Type de projet : {type_label}.\n"
            f"Contexte : application React/TypeScript d%#ployable sur Railway ou Vercel.\n"
            f"Texte UI en fran%�ais.\n\n"
            f"{PERSONALIZED_CONTENT_DIRECTIVE}\n\n"
            f"{build_toolbox_builder_context(plan)}"
            f"{prompt}"
        ...
        await _step(
            ...
            "coremind",
            "done",
            f"App React g%#n%#r%#e ��� {file_count} fichier(s), {code_chars} chars ,%V% {generation.model}",
            template="real_app",
            html_bytes=code_chars,
        )
        return {
            "generation": generation,
            # Pas d'aper%�u HTML : le code React n'est pas rendu directement.
            "preview_html": None,
        }

    # --- Template-first (ecommerce, r%#servation, app, desktop) : pas de dashboard premium ---
    ...
    if is_template_first_html_project(plan, generation_mode=generation_mode):
        ...
        await _step(
            ...
            "coremind",
            "start",
            "Reprise assemblage template sectoriel (%#vite template premium dashboard)�Ǫ",
        )
        ...
        logger.warning(
            "[CoreMindAI] reprise template-first %#chou%#e ��� fallback demo premium (%s)",
            plan.template,
        )

    # --- Mode "D%#mo client" (par d%#faut) : pipeline HTML premium TaskFlow ---
    await _step(cb, "coremind", "start", "G%#n%#ration HTML et seed adapt%#e au template�Ǫ")
    enriched = (
    ...
    await _step(
        ...
        "coremind",
        "done",
        f"HTML g%#n%#r%# ({document.html_bytes} octets, template {document.template})",
...
async def visionui_node(
    ...
    if not plan:
        return {"error": "%�tat pipeline incomplet (VisionUI)."}
    ...
    if generation_mode in ("real_app", "vitrine_next"):
        await _step(
            ...
            "visionui",
            "done",
            "Aper%�u HTML non applicable ��� %#tape ignor%#e.",
        )
        return {}
    ...
    if not html:
        await _step(cb, "visionui", "done", "Aucun HTML %� capturer ��� %#tape ignor%#e.")
        return {}
    ...
    await _step(
        ...
        "visionui",
        "start",
        "M%#dias toolbox (photos, ic%$%nes, illustrations) puis capture�Ǫ",
    )
    settings = _settings_from_config(config)
    ...
    await _step(
        ...
        "visionui",
        "done",
        result.preview.message or f"Aper%�u VisionUI ({source_label}).",
...
async def bughunter_node(
    ...
    if not generation or not plan:
        return {"error": "%�tat pipeline incomplet (g%#n%#ration manquante)."}

    await _step(cb, "bughunter", "start", "V%#rification du HTML g%#n%#r%#�Ǫ")
    settings = _settings_from_config(config)
    ...
    msg = (
        ...
        if report.ok
        else f"{len(report.issues)} probl%�me(s) : {', '.join(report.issue_codes[:6])}"
    )
    if literal_repaired and report.ok:
        msg = "Identit%# client inject%#e ��� HTML valid%#."
    elif literal_repaired:
        msg = (
            "Identit%# client inject%#e ��� "
            + (
                "alertes restantes non li%#es au client."
                if not client_literal_issues_only(report.issue_codes)
                else "v%#rification partielle."
            )
        )
...
async def autofix_node(
    ...
    if not report or not analysis or not plan or not generation:
        return {"error": "%�tat pipeline incomplet (autofix)."}
    ...
    if CLIENT_LITERAL_ISSUE_CODES.intersection(report.issue_codes):
        ...
        if profile.company_name and is_vitrine_html_project(
            ...
            await _step(
                ...
                "autofix",
                "done",
                "Identit%# client inject%#e (sans r%#g%#n%#ration LLM)."
                if report.ok
                else "Post-traitement client appliqu%#.",
    ...
    await _step(
        ...
        "autofix",
        "start",
        f"Correction automatique (boucle {loop_num}/{MAX_AUTOFIX_LOOPS})�Ǫ",
        loop=loop_num,
    )
    settings = _settings_from_config(config)
    tier = CodeGenComplexity(analysis.complexity.value)
...
async def testpilot_node(
    ...
    preview_html = state.get("preview_html")
    if not generation or not plan:
        return {"error": "%�tat pipeline incomplet (TestPilotAI)."}

    await _step(cb, "testpilot", "start", "Validation rendu, liens et scripts JS�Ǫ")
    settings = _settings_from_config(config)
    ...
    if report.ok:
        ...
        await _step(
            ...
            "testpilot",
            "done",
            f"Validation r%#ussie ��� {badge_label}.",
            ok=True,
    ...
    await _step(
        ...
        "testpilot",
        "done",
        f"{len(report.issues)} %#chec(s) de validation ��� "
        + (
...
async def playwright_node(
    ...
    await _step(cb, "playwright", "start", "Tests E2E Chromium (Playwright)�Ǫ")
    settings = _settings_from_config(config)
...
async def lighthouse_node(
    ...
    await _step(cb, "lighthouse", "start", "Audit Lighthouse (Performance, SEO, A11y)�Ǫ")
    settings = _settings_from_config(config)
...
async def export_node(
    ...
    cb = _callback_from_config(config)
    if state.get("error"):
        await _step(cb, "export", "done", "Export ignor%# (erreur pipeline).")
        return {}
    ...
    if not plan or not analysis or not generation:
        await _step(cb, "export", "done", "Export ignor%# (g%#n%#ration incompl%�te).")
        return {}

    generation_mode = state.get("generation_mode") or "client_demo"
    if plan.project_type == ProjectType.EXTENSION_NAVIGATEUR:
        await _step(cb, "export", "start", "Export ZIP extension Chrome�Ǫ")
    ...
    if personal and generation_mode == "real_app":
        await _step(
            ...
            "export",
            "start",
            "D%#ploiement Cloudflare Pages d%#di%# (URL *.pages.dev)�Ǫ",
        )
    elif generation_mode in ("real_app", "vitrine_next"):
        label = (
            "D%#ploiement site vitrine Next.js (Vercel)�Ǫ"
            if generation_mode == "vitrine_next"
            else "D%#ploiement application React (Railway / Vercel)�Ǫ"
        )
        await _step(cb, "export", "start", label)
    else:
        await _step(cb, "export", "start", "D%#ploiement automatique (Cloudflare / Railway / GitHub)�Ǫ")
    ...
    # Pas d'aper%�u HTML pour React / vitrine Next (%#vite export Cloudflare d%#mo).
    if generation_mode in ("real_app", "vitrine_next"):
        assembled_html = ""
        preview_html = ""
    else:
    ...
    await _step(
        ...
        "export",
        "done",
        result.message or "Export termin%#.",
        ok=result.success,
...
async def finalize_node(
    ...
    if not analysis or not generation or not architect:
        return {"error": "Pipeline termin%# sans r%#sultat exploitable."}
    ...
    await _step(cb, "finalize", "start", "Assemblage de la r%#ponse�Ǫ")
    settings = _settings_from_config(config)
    cfg = _configurable(config)
    ...
    planned = (
        ...
        else [f"{TEMPLATE_PROVIDER} ,%V% {TEMPLATE_MODEL} (template pr%#fabriqu%#)"]
    )
    ...
    if tp_report:
        summary = (
            "Validation TestPilotAI r%#ussie."
            if tp_report.ok
    ...
    await _step(cb, "finalize", "done", "G%#n%#ration termin%#e.")
...
def build_pipeline_graph() -> Any:
    ...
    Vitrine (client_demo / vitrine_next) :
      ...
      ��� Builder ��� VisionUI ��� BugHunter ��� �Ǫ ��� Export ��� Finalisation

    real_app : m%�me pr%#ambule visuel optionnel, puis Builder (LLM) ou CoreMind si %#chec.
    """
...
async def run_generation_pipeline(
    ...
    *,
    ...
    """
    Ex%#cute le pipeline complet et retourne le r%#sultat CoreMindRunResult.
    %�met des %#v%#nements step_* via on_event si fourni (SSE frontend).
    generation_mode : "client_demo" (d%#faut), "real_app" ou "vitrine_next".
    """
    ...
    try:
        ...
        if not result:
            raise ValueError("Le pipeline n'a pas produit de r%#sultat.")
        return result
    finally:
        if pid:
            try:
                ...
                logger.exception("flush_project_costs(%s) %#chou%#", pid)
